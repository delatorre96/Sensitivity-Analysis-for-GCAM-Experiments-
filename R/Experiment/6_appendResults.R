...
write_experiment_info <- function(con,
  ...
  info <- data.frame(
    ...
    perturbation_strategy = if (is.null(perturbation_strategy)) NA_character_ else perturbation_strategy,
    distribution = if (is.null(distribution)) NA_character_ else distribution,
    distribution_parameters = if (is.null(distribution_parameters)) NA_character_ else toJSON(distribution_parameters, auto_unbox = TRUE),
...
write_run_info <- function(con,
  ...
  info <- data.frame(
    ...
    delta =  if (is.null(delta)) NA_character_ else delta,
    stringsAsFactors = FALSE
  )
